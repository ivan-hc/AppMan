...
#### NOTE that this project has been interrupted, in favor of the [new base code](https://github.com/ivan-hc/AM-application-manager).
...
To work on "AM" is hardest, because each script costs several hours of search and study to create scripts that can made the program completelly autonomous. "AM" is the future of AppMan, also because each program is available at system level, so all the users of a PC can use the same program without problems.
