...
# Related projects
[EYA](https://github.com/ivan-hc/EYA-App-Manager), it is a new App Manager I've done, inspired by AppMan. The commands are less compared to AppMan, because there are less things to do. EYA is built to allow integration in the system and automatic updates for each application, so the final user must do nothing but just using the application.
